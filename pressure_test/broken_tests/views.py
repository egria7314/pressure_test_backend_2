...
def detect_broken_image(pk):
    ...
    clippath = os.path.join(local_path, clip.full_path.lower()[len(os.path.dirname(nas.location))+1:]) # +1 for replaced '/'
    ...
    # [TODO]
    subfolder_with_clipname = clip.full_path.lower()[len(os.path.dirname(nas.location))+1:].replace('/', '_') # +1 for replaced '/'
